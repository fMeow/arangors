...
impl<'a, C: ClientExt> Collection<C> {
  ...
  pub async fn load(&self, count: bool) -> Result<Info, ClientError> {
    ...
    let resp: Info = deserialize_response(self.session.put(url, body.to_string()).await?.body())?;
    Ok(resp)
  }
  ...
  pub async fn load_indexes(&self) -> Result<bool, ClientError> {
    ...
    let resp: ArangoResult<bool> = deserialize_response(self.session.put(url, "").await?.body())?;
    Ok(resp.unwrap())
  }
    ...
    T: Serialize + DeserializeOwned,
  {
    ...
    let resp: DocumentResponse<T> = deserialize_response(self.session.request(req).await?.body())?;
    Ok(resp)
  }
